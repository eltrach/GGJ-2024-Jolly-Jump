...
half4 FragmentBlade (Varyings input, half vFace : VFACE) : SV_Target
{
	float a=1;
    if(input.vertex.x>_RightValue || input.vertex.x<-_LeftValue) 
		a=0;
	half4 outpoutBlade = Fragment(input, vFace);
	if(_ActivateCutOff == 1) 
		outpoutBlade.a = a;
	return outpoutBlade;
}
