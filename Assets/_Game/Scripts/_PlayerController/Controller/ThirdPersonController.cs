...
namespace VTemplate.Controller
{
    public class ThirdPersonController : IHuman
    {
        ...
        private void Update()
        {
            if (PlayerRoot.HealthSystem.IsDead) canMove = false;
            if (canMove)
            {
                JumpAndGravity();
                GroundedCheck();
        ...
        public void Die()
        {
            ...
            canMove = false;
            PlayerRoot.HealthSystem.IsDead = true;
        }
        ...
        internal void EnterVehicle(BaseVehicle baseVehicle)
        {
            if (baseVehicle == null) return;
            PlayerRoot.StateSystem.SwitchState(GameState.InVehicle);
        }
    }
}